...
Watford_16 <- Primera_Segunda %>% filter(Equipo == "Watford", Temporada == "2015-16")  %>% mutate(Porcentaje_Por_Partido = total_puntos / 114 * 100) %>% mutate(across(6, ~ round(., 2)))
   ...
   #Añadiendo Columna de % de Proporsión

Porcentaje_Por_Partido <- c(Burnley_15$Porcentaje_Por_Partido, Burnley_16$Porcentaje_Por_Partido, QPR_15$Porcentaje_Por_Partido, QPR_16$Porcentaje_Por_Partido, Hull_15$Porcentaje_Por_Partido, Hull_16$Porcentaje_Por_Partido, Bournemouth_15$Porcentaje_Por_Partido, Bournemouth_16$Porcentaje_Por_Partido, Norwich_15$Porcentaje_Por_Partido, Norwich_16$Porcentaje_Por_Partido, Watford_15$Porcentaje_Por_Partido, Watford_16$Porcentaje_Por_Partido)

Primera_Segunda_2 <- Primera_Segunda[1:12, 1:5]

Primera_Segunda_2 <- data_frame(Primera_Segunda_2, Porcentaje_Por_Partido)

remove(Burnley_15, Burnley_16, QPR_15, QPR_16, Hull_15, Hull_16, Bournemouth_15, Bournemouth_16, Norwich_15, Norwich_16, Watford_15, Watford_16, Porcentaje_Por_Partido)
...
# Planteamiento del Problema
  ...
  El fútbol es un deporte a nivel profesional de alto rendimiento que se disputa entre dos conjuntos de 11 jugadores cada uno, esta disciplina conlleva contacto y cada integrante del juego ejerce un rol fundamental en el desempeño de sus respectivos equipos. La misión de estos jugadores es disputar y trasladar un balón utilizando cualquier parte del cuerpo exceptuando brazos y manos con la finalidad de encajarlo dentro de la portería o arco del conjunto rival, acción que se denomina gol y hará ganar el partido al equipo que al cabo de noventa minutos logre hacer más anotaciones.
 ...
 Sabiendo que marcar más goles que el rival o en otras palabras ganar, es determinante en cualquier competición, resulta interesante develar como se comportan diversos equipos, a nivel de resultados, al enfrentarse a situaciones que puedan involucrar un mejor o peor desenvolvimiento a lo largo de un partido o temporada.
 
Tomando en consideración que se tiene en estudio una base de datos relacionada con algunas ligas del fútbol europeo, que contiene información de partidos recopilados desde el año 2006 hasta el año 2021, y se puede apreciar quién jugaba de local, quién jugaba de visitante, los goles anotados y el equipo ganado.  
El presente trabajo estadístico tendrá como finalidad analizar dicho comportamiento, tomando como puntos de partida situaciones que puedan conllevar una ventaja o desventaja a la hora de marcar goles o ganar encuentros para los equipos que se estudiarán a continuación.
...
```{r, echo=FALSE}
ggplot(Datos1, aes(x=total_puntos, y=Equipo, fill=VS)) + geom_col(color="black") + labs(x = "Puntos Acumulados") + ggtitle("Comparativa de puntos entre los equipos de Segunda division") + theme(plot.title=element_text(hjust = 0.5, size=11)) + geom_text(aes(label=total_puntos), vjust=0, hjust=-0.3, size=3) + theme_bw()
...
```{r, echo=FALSE}
ggplot(Datos2, aes(x=total_puntos, y=Equipo, fill=VS)) + geom_col(color="black") + labs(x = "Puntos Acumulados") + ggtitle("Comparativa de puntos entre los equipos de primera division") + theme(plot.title=element_text(hjust = 0.5, size=11)) + geom_text(aes(label=total_puntos), vjust=0, hjust=-0.3, size=3) + theme_bw()
...
En el presente Trabajo estadistico pudimos observar primeramente como nada esta definido en este deporte, los partidos hay que jugarlos y ganarlos; muchos factores pueden influir en el desenvolvimiento de un partido y como lo pudimos notar los equipos crecen al jugar en sus respectivas casas, equipos suben y bajan de divicion constantemente pero siempre se mantiene la competitividad. Los integrantes de un equipo y su direccion son parte fundamental del rendimiento, un equipo puede tener mas experiencia al competir que su rival pero lo unico que genera la victoria es llevarse el marcador.
